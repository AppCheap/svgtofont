...
import { log } from './log';
import { generateIconsSource, generateReactIcons, generateReactNativeIcons, generateFlutterIcons } from './generate';
...
export type SvgToFontOptions = {
    ...
    /**
     * Output: `./dist/flutter/`, SVG generates `flutter` component.
     */
    outFlutter?: boolean;
    /**
    ...
    try {
      ...
      `);
        ...
        if (options.outFlutter) {
            generateFlutterIcons(options, unicodeObject);
            log.log(`${color.green('SUCCESS')} Created Flutter Components. `);
        }
